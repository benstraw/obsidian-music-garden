...
func main() {
	...
	case "review":
		runReview(args, paths)
	case "export":
		runExport(args, paths)
	case "doctor":
...
Usage:
  ...
  music-garden review <action>                List, show, approve, or reject publication decisions
  music-garden export --contract public-v1    Build the privacy-safe public consumer contract
...
func resolveRuntimePaths() runtimePaths {
	...
	p := runtimePaths{
		...
		genresPath:   preferredCatalogPath(cwd, "genres.json"),
		artistsPath:  preferredCatalogPath(cwd, "artists.json"),
		releasesPath: preferredCatalogPath(cwd, "releases.json"),
		reviewsDir:   filepath.Join(cwd, "data", "reviews"),
		publishedDir: filepath.Join(cwd, "data", "published", "v1"),
	...
	p.reviewsDir = filepath.Join(p.dataRoot, "reviews")
	p.publishedDir = filepath.Join(p.dataRoot, "published", "v1")
	...
	if genresPath := strings.TrimSpace(os.Getenv("MUSIC_GENRES_PATH")); genresPath != "" {
		...
		p.genresPath, p.genresFallback = chooseStateCatalogPath(absState, "genres.json", p.genresPath)
	}
	if artistsPath := strings.TrimSpace(os.Getenv("MUSIC_ARTISTS_PATH")); artistsPath != "" {
		absArtistsPath, err := filepath.Abs(artistsPath)
		if err != nil {
		...
		p.artistsPath, p.artistsFallback = chooseStateCatalogPath(absState, "artists.json", p.artistsPath)
	}
	if releasesPath := strings.TrimSpace(os.Getenv("MUSIC_RELEASES_PATH")); releasesPath != "" {
		absReleasesPath, err := filepath.Abs(releasesPath)
		if err != nil {
		...
		p.releasesPath, p.releasesFallback = chooseStateCatalogPath(absState, "releases.json", p.releasesPath)
	}

	return p
}

func preferredCatalogPath(root, name string) string {
	preferred := filepath.Join(root, "data", "catalog", name)
...
func chooseStateCatalogPath(stateDir, name, fallbackPath string) (string, bool) {
	preferred := filepath.Join(stateDir, "data", "catalog", name)
	if _, err := os.Stat(preferred); err == nil {
		return preferred, false
	}
	legacy := filepath.Join(stateDir, "data", name)
...
func emitFallbackWarnings(paths runtimePaths, cmd string) {
	...
	playsUsed := cmd == "collect" || cmd == "repair-plays" || cmd == "backfill-play-artists" || cmd == "weekly" || cmd == "daily" || cmd == "catch-up" || cmd == "persona" || cmd == "genre-backfill" || cmd == "export" || cmd == "doctor"
	if playsUsed && paths.playsFallback {
		fmt.Fprintf(os.Stderr, "warning: MUSIC_STATE_DIR is set but %s was not found; falling back to %s\n", filepath.Join(paths.stateDir, "data", "plays"), paths.playsDir)
	}

	genresUsed := cmd == "collect" || cmd == "repair-plays" || cmd == "backfill-play-artists" || cmd == "weekly" || cmd == "daily" || cmd == "catch-up" || cmd == "persona" || cmd == "genre-backfill" || cmd == "image-backfill" || cmd == "export" || cmd == "review"
...
// saveCatalog persists only authoritative canonical stores. Raw responses are
// runtime caches; normalized and aggregated views are now built in memory by
// the public exporter instead of being persisted as parallel data trees.
...
func defaultGenreTaxonomyPath(paths runtimePaths) string {
	preferred := filepath.Join(paths.dataRoot, "catalog", "genre-taxonomy.json")
	if _, err := os.Stat(preferred); err == nil {
		return preferred
	}
...
func runBackfillPlayArtists(args []string, paths runtimePaths) {
	...
	var tracks []models.TrackDetails
	if len(trackIDs) > 0 {
	...
	albumIDs := []string{}
	for _, play := range rewritten {
		if play.AlbumID != "" && !albumSeen[play.AlbumID] && albumEditionNeedsBackfill(catalog, play) {
			albumSeen[play.AlbumID] = true
			albumIDs = append(albumIDs, play.AlbumID)
		}
	}
	sort.Strings(albumIDs)
	albums, _, err := fetch.GetAlbumsBatchRaw(c, albumIDs)
	if err != nil {
		fmt.Fprintln(os.Stderr, "album edition backfill fetch error:", err)
		os.Exit(1)
	}
	updatedEditions := 0
	for _, album := range albums {
		if genres.MergeAlbumDetails(catalog, album) {
			updatedEditions++
		}
	}
	if *dryRun {
		fmt.Printf("Dry run: candidate track IDs=%d updated plays=%d album editions=%d\n", len(trackIDs), changed, updatedEditions)
		return
	}
	...
	saveCatalog(paths, catalog)
	fmt.Printf("Backfilled metadata for %d play(s), %d track ID(s), and %d album edition(s).\n", changed, len(trackIDs), updatedEditions)
}

func albumEditionNeedsBackfill(catalog *genres.Catalog, play models.Play) bool {
	...
	for _, edition := range release.Editions {
		if edition.SpotifyAlbumID == play.AlbumID {
			return edition.TotalTracks == 0 || len(edition.Tracks) < edition.TotalTracks
		}
	}
	return true
}
...
func generateDailyNote(allPlays []models.Play, date time.Time, overwrite bool, artistMetadata map[string]genres.ArtistRecord, outputRoots ...string) {
	outputRoot := vaultPath()
	if len(outputRoots) > 0 && strings.TrimSpace(outputRoots[0]) != "" {
