...
func CandidateTrackIDs(existing []models.Play, opts PlayArtistsOptions) []string {
	...
	for _, play := range existing {
		...
		if play.AlbumID != "" && play.TrackNumber > 0 && play.AlbumTotalTracks > 0 {
			continue
		}
...
func applyTrackDetails(play models.Play, track models.TrackDetails) models.Play {
	...
	if play.AlbumType == "" {
		play.AlbumType = track.AlbumType
	}
	if play.AlbumReleaseDate == "" {
		play.AlbumReleaseDate = track.AlbumReleaseDate
	}
	if play.AlbumTotalTracks == 0 {
		play.AlbumTotalTracks = track.AlbumTotalTracks
	}
	if len(play.AlbumImages) == 0 {
		play.AlbumImages = append([]models.ArtistImage(nil), track.AlbumImages...)
	}
	if play.DiscNumber == 0 {
		play.DiscNumber = track.DiscNumber
	}
	if play.TrackNumber == 0 {
		play.TrackNumber = track.TrackNumber
	}
	if play.DurationMS == 0 {
		play.DurationMS = track.DurationMS
	}
	if len(play.AlbumArtists) == 0 {
		play.AlbumArtists = append([]models.PlayArtist(nil), track.AlbumArtists...)
	}
